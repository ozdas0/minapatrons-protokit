...
import { MinaPatron, offchainState } from '../build/src/minapatrons.js';
...
let minapatrons_contract = new MinaPatron(zkAppAddress);
minapatrons_contract.offchainState.setContractInstance(minapatrons_contract);
...
try {
  tx = await Mina.transaction({ sender: feepayerAddress, fee }, async () => {
    ...
    await minapatrons_contract.deploy();
  });
  await tx.prove();
